...
interface CommentWithMovieInfo extends CommentType {
    movieId: string;
    movieTitle: string;
}
...
rounded-full peer peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 dark:after:border-gray-600 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
    </label>
  </div>
);
 ...
 */
  ...
  const handleOpenPermissionsModal = (admin: AdminUser) => {
    ...
    // Use File 2's permission structure
    setTempPermissions({
        canAddContent: admin.permissions.canAddMovies || false,
        canEditContent: admin.permissions.canEditMovies || false,
        canDeleteContent: admin.permissions.canDeleteMovies || false,
        canManageComments: false, // This is new from File 2, default to false
        canLiveEdit: admin.permissions.canLiveEdit || false,
    } as any); // Use 'as any' to match the new UI structure
  };

  const handleSavePermissions = () => {
    if (editingPermissionsFor && tempPermissions) {
        // Map back to File 1's permission structure if needed, or just save the new structure
        // This logic saves the *new* structure (from File 2's UI)
        const newPermissions: AdminPermissions = {
             canAddMovies: (tempPermissions as any).canAddContent || false,
             canEditMovies: (tempPermissions as any).canEditContent || false,
             canDeleteMovies: (tempPermissions as any).canDeleteContent || false,
             canLiveEdit: (tempPermissions as any).canLiveEdit || false,
             // Add other permissions if they exist in the type
             ...(tempPermissions as any) // Keep any other perms
        };

      if (updateAdminPermissions(editingPermissionsFor.username, newPermissions)) {
        setAdmins(getAdmins());
  ...
  const handleCommentSearch = (e: React.FormEvent) => {
    ...
    const results = allComments.filter(
      (c) => (c.username || (c as any).name).toLowerCase() === commentSearch.trim().toLowerCase()
    );
  ...
  try {
    ...
    const res = await fetch("/.netlify/functions/addComment", {
      ...
      body: JSON.stringify({
        ...
        parentId: adminReplyingTo.id || (adminReplyingTo as any)._id, // kis 
      }),
    });
  ...
  const handleStartEdit = (movie: Movie) => {
    ...
    setSelectedManagedMovie(null); // Close the modal
  ...
  const handleDownloadLinkChange = (
    ...
    const updated = formState.downloadLinks ?
[...formState.downloadLinks] : [];
  ...
  const handleTelegramLinkChange = (
    ...
    const updated = formState.telegramLinks ?
[...formState.telegramLinks] : [];
...
0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMovieId) {
        // Check permissions using File 2's names
         if (!hasPermission('canEditContent' as any)) {
        ...
        // Check permissions using File 2's names
         if (!hasPermission('canAddContent' as any)) {
  ...
  const handleDeleteMovie = async (movie: Movie) => {
    // Check permissions using File 2's names
    if (!hasPermission('canDeleteContent' as any)) {
    ...
    try {
      ...
      await fetchMovies();
      alert("Deleted.");
      setSelectedManagedMovie(null); // Close modal
    } catch (err: any) {
      console.error(err);
 ...
 // --- End Resizable Sidebar Logic ---
    ...
    // Use File 1's logic for displaying comments
    const commentsToDisplay = (filteredComments.length > 0 ? filteredComments : allComments);

    return (
        <div className="container mx-auto p-4 animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    {/* Use File 2's permission name */}
                    {hasPermission('canManageComments' as any) && (
                        <button 
            ...
            {hasPermission('canManageComments' as any) && (
                ...
                >
                    ...
                    <div className="flex-grow p-4 space-y-4 overflow-y-auto">
                        ...
                        {commentsToDisplay.length > 0 ? commentsToDisplay.map((comment: any) => (
                            <div key={comment.id || comment._id} className="p-3 bg-light-card dark:bg-brand-card rounded-lg shadow-sm animate-fade-in">
                                <div className="flex justify-between items-center text-xs">
                                    {/* Use File 1's data structure */}
                                    <p className="font-bold text-light-text dark:text-brand-text">{comment.username || (comment as any).name || "Unknown User"}</p>
                                    <p className="text-light-text-secondary dark:text-brand-text-secondary">
                                        {comment.createdAt ? new Date(comment.createdAt).toLocaleString() : ((comment as any).timestamp ? new Date((comment as any).timestamp).toLocaleString() : "Unknown Time")}
                                    </p>
                                </div>
            ...
            {/* Use File 2's permission name */}
            {hasPermission('canAddContent' as any) && (
            <div className="bg-light-card dark:bg-brand-card p-6 rounded-lg mb-8 shadow-md">
                ...
                <div className="border border-gray-300 dark:border-gray-600 p-4 rounded-md mb-6 space-y-3 bg-light-bg/50 dark:bg-brand-bg/50">
                    ...
                    <div className="flex gap-2">
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && handleTmdbSearch(e as any)}
                    ...
                    {tmdbResults.length > 0 && (
                        ...
                        {tmdbResults.map(result => (
                            <div key={result.id} onClick={() => handleSelectTmdb(result)} className="cursor-pointer group text-center animate-fade-in">
                            <img
                                src={result.poster_path ? `${TMDB_IMAGE_BASE_URL}w200${result.poster_path}` : 'https://via.placeholder.com/200x300.png?text=No+Image'}
                                alt={(result as any).title || (result as any).name}
                                className="rounded-md shadow-md group-hover:opacity-75 transition-opacity aspect-[2/3] object-cover w-full"
                            />
                            <p className="text-sm mt-2 font-medium text-light-text dark:text-brand-text truncate">{(result as any).title || (result as any).name}</p>
                            <p className="text-xs text-light-text-secondary dark:text-brand-text-secondary">
                                {result.media_type.toUpperCase()} ({ ((result as any).release_date || (result as any).first_air_date)?.substring(0, 4) })
                            </p>
                            </div>
                        ))}
                        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        {/* Replaced File 2's upload with File 1's text input */}
                        <label className="grid gap-1">
                            <span className="block text-sm font-medium text-light-text-secondary dark:text-brand-text-secondary mb-1">Poster URL</span>
                            <input
                                name="posterUrl"
                    ...
                    <input type="number" name="rating" value={formState.rating || ''} onChange={handleInputChange} placeholder="Rating (0-10)" step="0.1" min="0" max="10" className={inputClass} />
                    ...
                    {formState.category === 'webseries' ? (
                       <fieldset className="border border-gray-300 dark:border-gray-600 p-4 rounded-md space-y-4">
                ...
                >
                    ...
                    >
                        ...
                        <div className="w-full flex flex-col gap-3">
                             {/* Use File 2's permission name but File 1's function */}
                             {hasPermission('canEditContent' as any) && (
                                <button 
                            ...
                            {/* Use File 2's permission name but File 1's function */}
                            {hasPermission('canDeleteContent' as any) && (
                                <button 
                                    onClick={() => handleDeleteMovie(selectedManagedMovie)} 
            ...
            {adminReplyingTo && (
                ...
                >
                    ...
                    >
                        {/* Use File 1's data structure */}
                        <h3 className="text-lg font-bold text-light-text dark:text-brand-text mb-2">Replying to {adminReplyingTo.username || (adminReplyingTo as any).name}</h3>
            ...
            {editingPermissionsFor && tempPermissions && (
                ...
                >
                    ...
                    >
                        ...
                        {/* Use File 2's permission UI structure */}
                        <div className="space-y-3">
                            <PermissionToggle 
                                label="Add Content"
                                isChecked={!!(tempPermissions as any).canAddContent}
                                onChange={(isChecked) => setTempPermissions(p => ({...p!, canAddContent: isChecked}) as any)}
                            />
                             <PermissionToggle 
                                label="Edit Content"
                                isChecked={!!(tempPermissions as any).canEditContent}
                                onChange={(isChecked) => setTempPermissions(p => ({...p!, canEditContent: isChecked}) as any)}
                            />
                             <PermissionToggle 
                                label="Delete Content"
                                isChecked={!!(tempPermissions as any).canDeleteContent}
                                onChange={(isChecked) => setTempPermissions(p => ({...p!, canDeleteContent: isChecked}) as any)}
                            />
                             <PermissionToggle 
                                label="Manage Comments"
                                isChecked={!!(tempPermissions as any).canManageComments}
                                onChange={(isChecked) => setTempPermissions(p => ({...p!, canManageComments: isChecked} as any))}
                            />
                            <PermissionToggle 
                                label="Live Edit Content"
                                isChecked={!!(tempPermissions as any).canLiveEdit}
                                onChange={(isChecked) => setTempPermissions(p => ({...p!, canLiveEdit: isChecked}) as any)}
                            />
                        </div>
